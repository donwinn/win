...
body = data.replace("r.fx=s==null\?\"\":s","r.fx=s==null?\"\":s.replace(s.split(\"\.\")[0],\"https://long\")");//播放地址
data = body.replace("bhW\(\)\)\),!0\)","bhW())),!0)\ns=JSON.parse(q)\nif(s.data.aw_mv_perm==0){s.data.aw_mv_perm=1,s.data.vip_level=11}\nq=JSON.stringify(s)");//会员解锁暗网限制
